...
class NavTester(object):
    ...
    def __init__(self, options, scene_id):
        ...
        self.options.episodes_root = "habitat-lab/data/datasets/pointnav/mp3d/"+self.options.test_set+"/"
        ...
        for n in range(self.options.ensemble_size):
            ...
            # 通过nn.DataParallel的构造函数进行包装，就能够将其转换为一个支持在多个GPU上并行运行的版本。
            # 这样一来，在模型进行前向传播、反向传播等计算时，PyTorch会自动地将数据分发到多个GPU上，并将结果合并起来。
        ...
        model_path = self.options.root_path + "local_policy/" + model_ext
